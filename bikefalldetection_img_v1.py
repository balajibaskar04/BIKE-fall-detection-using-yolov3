...
#image_BGR = cv2.imread(r'C:\Users\balaji_baskar\Downloads\gray1.jpg')
image_BGR = cv2.imread(r'C:\Users\balaji_baskar\Downloads\images.jpg')
...
# layers_names_output = \
#     [print (i) for i in network.getUnconnectedOutLayers()]
# # Check point
# print()
...
for result in output_from_network:
    ...
    for detected_objects in result:
        ...
        if confidence_current > probability_minimum:
            ...
            # print("***********x", x_min, y_min, box_width, box_height)
...
if len(results) > 0:
    ...
    for i in results.flatten():
        ...
        if labels[int(class_numbers[i])] == 'motorbike':
            print("***********x", x_min, y_min, box_width, box_height)
            cv2.rectangle(image_BGR, (x_min, y_min),
                          (x_min + box_width, y_min + box_height),
                          colour_box_current, 2)

            center_x = int((x_min + x_min + box_width) / 2)
            center_y = int((y_min + y_min + box_height) / 2)
            center = (center_x, center_y)



            line_1 = x_min, y_min
            line_2 = box_height, box_width

            # cv2.line(image_BGR, line_1, line_2, (255, 0, 0), thickness=3, lineType=cv2.LINE_8)

            cv2.line(image_BGR, (center_x, y_min), (center_x, y_min + box_height), (255, 0, 0), 2)


            # def calculate_angle(x_min, y_min, box_width, box_height):
                # # Calculate the center of the bounding box
                # center_x = x_min + box_width / 2
                # center_y = y_min + box_height / 2

                # Calculate the angle of the bounding box with respect to horizontal axis
            angle = math.degrees(math.atan2(box_height, box_width))

            print (angle)

            angle_threshold = 5
            #def detect_objects_at_angle_45_degrees(x_min, y_min, box_width, box_height, angle_threshold=5):
            #angle = calculate_angle(x_min, y_min, box_width, box_height)

            if abs(angle - 45) <= angle_threshold:
                print("True")
                print("Object detected at 45-degree angle!")
            else:
                print("false")
                print("Object not at a 45-degree angle.")


            # Tolerance level for angle deviation from 45 degrees

            # if detect_objects_at_angle_45_degrees(x_min, y_min, box_width, box_height, angle_threshold):
            #
            # else:
            #
